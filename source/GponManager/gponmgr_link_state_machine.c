...
ANSC_STATUS gpon_disable_veip_interface(GPON_LINK_SM_CTRL_T* gpon_sm_ctrl)
{
    ...
    {
#if defined(WAN_MANAGER_UNIFICATION_ENABLED)
	  //  CcspTraceError(("ARUNLOG: Trace:%s  Msg: Notify WanMager by settung PhyStatus INTERFACE_DOWN\n",__FUNCTION__));
...
#else
	  //  CcspTraceError(("ARUNLOG: Trace:%s  Msg: Notify  EthernetAgent by setting FALSE\n",__FUNCTION__));
